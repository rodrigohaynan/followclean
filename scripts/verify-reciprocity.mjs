...
assert.equal(candidate({})[0].classification, "pre_priority", "count-only candidates remain visible but never enter confirmed priority");
assert.match(candidate({})[0].reasons.join(" "), /segunda conferência pendente/);
const confirmed = {
...
assert.equal(candidate(confirmed, "2026-09-25T08:00:00Z")[0].classification, "pre_priority", "new import invalidates old negative confirmation");
