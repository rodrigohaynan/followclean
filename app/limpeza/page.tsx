...
import { ArrowLeft, HardDrive, UsersRound } from "lucide-react";
    ...
    <main className="min-h-screen pb-16">
      <header className="mx-auto flex max-w-6xl items-center justify-between px-6 py-6">
        <Link href="/" className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">Fila inteligente</p>
          </div>
        </Link>
        <Link
          href="/dashboard"
      ...
      <section className="mx-auto max-w-6xl px-6 pt-8">
        <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            <p className="text-sm font-bold uppercase tracking-[0.16em] text-blue-600">Limpeza inteligente</p>
            <h1 className="mt-2 text-4xl font-black tracking-[-0.035em] text-slate-950">
              Regras, protegidos e fila de revisão
            </h1>
            <p className="mt-3 max-w-2xl leading-7 text-slate-600">
              O FollowClean aplica as regras ao snapshot mais recente e monta uma fila local. Você continua no controle de cada decisão.
            </p>
          </div>
          <div className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-3 text-xs font-semibold text-slate-600 shadow-sm">
            <HardDrive size={16} className="text-emerald-600" /> Regras salvas neste dispositivo
          </div>
        </div>
